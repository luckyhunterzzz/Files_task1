...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String mainDirGames = "F:\\Programms\\IDEA-projects\\homeprojects\\files\\files-installed\\Games\\";
        StringBuilder sb = new StringBuilder();

        String[] dirGames = { "src", "res", "savegames", "temp" };
        String[] dirSrc = {"main", "test" };
        String[] dirRes = {"drawables", "vectors", "icons"};
//        String[] filMain = {"Main.java", "Utils.java" };
//        String[] filTemp = {"temp.txt" };

        for (int i = 0; i < dirGames.length; i++) {
            File newDir = new File((mainDirGames + dirGames[i]));
            if (newDir.mkdirs()) {
        ...
        for (int i = 0; i < dirSrc.length; i++) {
            File newDir = new File((mainDirGames + "src\\" + dirSrc[i]));
            if (newDir.mkdirs()) {
        ...
        for (int i = 0; i < dirRes.length; i++) {
            File newDir = new File((mainDirGames + "res\\" + dirRes[i]));
            if (newDir.mkdirs()) {
